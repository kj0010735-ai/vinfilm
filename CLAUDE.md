...
- `ledger/index.html` — **STUDIO FINANCE (재무 장부) 앱**. 원래 GPT(ChatGPT 캔버스/앱, `*.chatgpt.site` 도메인)에서 "STUDIO FINANCE · 스튜디오의 돈, 한눈에." 컨셉으로 만들던 걸 로그인 게이트 때문에 그대로 가져올 수 없어서, 스크린샷으로 받은 레퍼런스를 보고 이 저장소의 바닐라 HTML/JS + works/index.html 패턴(로그인 게이트, `pushToCloud` diff-sync, `IS_LOCAL_DEV` 안전장치)으로 새로 재현한 것. **디자인은 VINFILM STUDIO 다른 페이지들의 다크 웜톤이 아니라 GPT 레퍼런스 그대로 라이트 테마 + 네이비/블루 포인트 컬러**(`--sf-navy`/`--sf-blue` 등, `sf-` 클래스 프리픽스)를 쓴다 — 이 앱만의 독립된 브랜드("STUDIO FINANCE")라 의도적으로 다르게 감. 탭 구성: 전체 장부 / 매출 / 인건비 / 지출 / 신고 관리 / 디지털 월세.
  ...
  - **디지털 월세 탭(구독 관리)**: 거래(entries)와 완전히 별도 데이터(`ledgerSubscriptions`) — 서비스명·요금제·금액·통화(KRW/USD)·결제주기(월간/연간)·다음 결제일·상태(사용중/일시중지/해지)·결제수단. **원화·달러는 환율 변환 없이 통화별로 따로 합산**한다(메인 장부와 달리 여기는 의도적으로 환율 계산을 안 함 — GPT 레퍼런스 원본 문구: "원화·달러는 환율 변환 없이 따로 표시해"). 월 환산(연간 결제는 12로 나눔)·연간 예상·7일 내 결제 예정 건수를 카드로 보여주고, "빠르게 추가" 칩(Suno/ChatGPT/Claude/Adobe)으로 서비스명만 미리 채운 등록 모달을 바로 연다.
  - 재무 데이터라 guest 모드 없음, **owner(`kj0010735@gmail.com`)만 접근** — 로그인은 하되 그 이메일이 아니면 "접근 권한이 없어요" 화면만 보여주는 클라이언트 UI 게이트이고, 실제 방어선은 여전히 firestore.rules의 `isOwner()`. Firestore 컬렉션은 `ledgerEntries` + `ledgerSubscriptions` (둘 다 works의 `pushToCloud` diff-sync 패턴 그대로). `IS_LOCAL_DEV` 가드도 처음부터 동일하게 적용돼 있어 로컬 테스트가 운영 데이터를 건드리지 않는다.
