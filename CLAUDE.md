...
**설정(⚙️) 모달 + 화이트 테마**: 사이드바 하단 "설정"(guest 포함 모두 보임, `open-settings`)에서 블랙/화이트 테마를 고른다. `S.settingsOpen`/`settingsModalHtml()`(전역 `render()`에서 스케줄 모달처럼 항상 렌더), 선택값은 `localStorage['vinfilm-works-theme']`(기기별)에 저장하고 `<html data-theme="light|dark">`로 적용한다 — `<head>` 맨 위 인라인 스크립트가 렌더 전에 속성을 먼저 세팅해서 새로고침 때 깜빡임이 없다. **화이트 테마는 `:root[data-theme="light"]`가 색 변수만 덮어써서 구현**(`--bg/--surface/--text/--accent/--row-bg…`)하므로, 새 CSS를 쓸 때 색을 `#hex`로 하드코딩하지 말고 변수를 쓸 것(안 그러면 화이트에서 혼자 검게 남는다). 프로젝트 행 카드 색도 그래서 `--row-bg`/`--row-bg-hover`/`--row-border`/`--row-title`/`--row-text` 변수로 뺐다. 공개 홈페이지·admin·ledger(이미 자체 라이트 테마)는 이 설정과 무관.
