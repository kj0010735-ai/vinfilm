...
- **행을 체크박스로 다중 선택해서 일괄 상태 변경/삭제할 수 있다**: 행 맨 왼쪽(`.proj-row-check-cell`)에 체크박스가 있고, 헤더 행에도 현재 화면에 보이는 항목 전체를 선택하는 체크박스가 있다(`data-role="proj-select-all"`/`shared-select-all`, `data-ids`에 담긴 콤마구분 id 목록을 토글). 선택된 게 하나라도 있으면 리스트 위에 `.proj-bulk-bar`가 뜨는데, 상태별 일괄 변경 버튼(`PROJECT_COLUMNS`/`SHARED_STATUSES` 순회)과 일괄 삭제·선택 해제 버튼이 있다 — 개별 행의 상태 변경/삭제와 동일하게 `saveProjItems`/`saveSharedProjItems`를 한 번에 호출하고 끝나면 선택을 비운다. 선택 상태(`S.proj.selectedIds`/`S.shared.selectedIds`)는 하위 프로젝트로 드릴다운하거나 상위로 나갈 때(`proj-drill-in`/`proj-go-parent` 등) 초기화된다 — 안 보이는 항목이 선택된 채로 남아있는 걸 방지. 공유 프로젝트는 게스트 모드일 때 체크박스/일괄바 자체가 렌더링 안 됨(다른 쓰기 액션과 동일한 제한). 모바일 폭에서는 드래그 핸들과 마찬가지로 체크박스 컬럼도 숨김.
- **프로젝트에 상태와 별개로 커스텀 색상을 지정할 수 있다**: 수정 모달에 `project.color` 필드를 고르는 원형 스와치 피커가 있다(장비 카테고리에서 쓰던 `CATEGORY_COLOR_PALETTE` 14색을 그대로 재활용 + "기본"(무색) 옵션). 네이티브 라디오 버튼 + `:has(input:checked)` CSS만으로 선택 상태를 표시해서(`.eq-status-option`과 같은 패턴) 별도 JS 없이 `FormData`로 저장됨. 색이 지정되면 리스트 행과 상위 요약 카드 왼쪽에 `border-left:3px solid {color}`로 표시되고(테두리 두께만큼 `padding-left`를 보정), 지정 안 하면 평소처럼 테두리 없음 — 상태(진행중/완료 등) 색과는 별개로 순수히 사용자가 구분하려고 붙이는 색이다. 프로젝트/공유 프로젝트 양쪽 다 동일하게 적용됨.
