...
**works 앱의 기본 테마는 '화이트'(깨끗한 흰색 카드 + `#f6f7f9` 배경)이고, 설정(⚙️)에서 블랙으로 바꿀 수 있다.** 처음엔 홈페이지와 톤을 맞춘 다크 웜톤이었지만 "홈페이지 톤앤매너는 안 맞춰도 되니 화이트로, 더 고급스럽게"라는 요청으로 기본값을 화이트로 뒤집었다 — 그래서 이 문서 위쪽의 "다크 웜톤" 설명은 **블랙 테마 쪽 값**이다. 사이드바 하단 "설정"(guest 포함, `open-settings`)의 모달(`S.settingsOpen`/`settingsModalHtml()`, 전역 `render()`에서 스케줄 모달처럼 항상 렌더)에서 고르고, 선택은 `localStorage['vinfilm-works-theme']`(기기별, 없으면 light)에 저장해 `<html data-theme="light|dark">`로 적용한다 — `<head>` 맨 위 인라인 스크립트가 렌더 전에 속성을 세팅해 깜빡임이 없다. 화이트는 `:root[data-theme="light"]`가 색 변수만 덮어써서 구현하므로 **새 CSS에서 색을 `#hex`로 하드코딩하지 말고 변수를 쓸 것**(`--bg/--surface/--border/--border-strong/--text*/--accent*/--shadow-sm|md|lg/--row-*`). 공개 홈페이지·admin·ledger는 이 설정과 무관.
